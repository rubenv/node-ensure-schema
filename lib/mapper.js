...
var TableMapper = (function () {
    ...
    function fieldNeedsUpdate(current, desired) {
        if (!this._mapper.backend.typeMatches(current.type, desired.type)) {
            return true; 
        }
    ...
    TableMapper.prototype.field = function (name, type, options) {
        ...
        if (this._isNew) {
            ...
            if (fieldNeedsUpdate.call(this, this._tableSchema.fields[name], fieldDef)) {
...
var Mapper = (function () {
    function Mapper(backend, db, schemaDefs, cb) {
        var self = this;

        if (!Array.isArray(schemaDefs)) {
    ...
    Mapper.prototype._alterField = function (table, fieldDef) {
        var self = this;
        this._queue.push(function (cb) {
            self.backend.alterField(self.db, table, fieldDef, cb);
        });
    };
